...
// //`?`기준으로 앞의 평가대상이 undefined || null 이 아니면 처리를 계속 이어가고
...
// Array 객체: 하나의 변수에 여러개의 값을 순차적으로 저장할 수 있는 데이터 구조
// 여러 종류의 데이터 타입을 저장 가능
// 배열의 길이 동적으로 변화
// 각 요소는 0부터 시작하는  인덱스를 가진다.
// 데이터 타입은 여러개가 되도 무관하나 되도록이면 한 종류만 쓰자
...
// start만 지정 했을 때 
// start가 양수 였을 때는, 인덱스가 start인 요소부터 모두 제거하고 반환
const resultSpliceStartPositive = arr8.splice(3);
console.log(arr8,resultSpliceStartPositive);

// start만 지정 했을 때 
// start가 음수 였을 때는, 배열의 끝 요소부터 start만큼의 요소를 제거히고 잘린 요소를 배열로 반환
...
const resultSpliceStartArgs = arr11.splice(1, 1, 100);
console.log(arr11, resultSpliceStartArgs);

// join(separator): 배열의 요소를 구분자로 연결한 문자열을 만들어서 변환
const arr12 = [1, 2, 3, 4, 5];
const resultJoin = arr12.join(':');
console.log(arr12, resultJoin);

//sort(): 원본 배열의 요소를 문자열로 변환 후 오름차순 정렬을 해서 만든 배열을 변환
...
// filter(callback): 배열의 모든 요소에 대해 콜백 함수를 반복하여 실행한 후, 조건에 만족한 요소만 모아 새로운 배열로 반환
